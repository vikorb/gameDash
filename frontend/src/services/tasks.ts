...
export async function fetchDailyTasks(userId: number): Promise<DailyTasksResponse> {
...
export async function fetchTaskHistory(userId: number, limitDays = 30): Promise<TaskHistoryResponse> {
  const { data } = await api.get<TaskHistoryResponse>('/tasks/history', {
    params: { userId, limitDays },
  })
  return data
}
