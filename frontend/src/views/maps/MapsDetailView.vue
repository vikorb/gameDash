          ...
          <div class="gallery__overlay">
            <div class="gallery__overlay-left">
              ...
              <div class="gallery__creator-line">
                ...
                {{ t('maps.card.by') }}
                <strong>{{ map.creator.username }}</strong>
        ...
        <div class="detail-actions-bar__votes">
          ...
          <div class="action-share-wrap">
            <button
              type="button"
              class="action-share"
              :aria-label="t('maps.detail.share')"
              @click="handleShare"
            >
              <svg viewBox="0 0 24 24" aria-hidden="true">
...
const pillStatus = computed(() => {
  if (map.value?.status === 'stable') return 'visible'
  if (map.value?.status === 'beta') return 'review'
  return 'draft'
})
...
const launchStep = ref(0) // 0=idle 1=connecting 2=loading 3=done
...
.test-btn--step3 {
  ...
  box-shadow: 0 12px 26px -14px rgba(29, 106, 79, 0.6);
}
...
/* Maps btn helper (reused from browse) */
.maps-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
