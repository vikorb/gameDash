...
import { findCompatiblePlayers, distributeToTeams, broadcastRoomUpdate } from './roomMatcher';
...
export async function runMatchmakingTick(manager: IManager, io: Server) {
  ...
  for (const room of manager.rooms.filter((r: MatchmakingRoom) => r.status === 'searching')) {
    ...
    const candidates = findCompatiblePlayers(manager.players, roomAvg).slice(0, needed);
    if (candidates.length > 0) {
      ...
      if (room.getAllPlayers().length === 8) {
        ...
        room.readyTimeout = setTimeout(() => launchGame(room, io, manager.rooms), 60000);
  ...
  if (manager.players.length >= 2) {
    const base = manager.players[0];
    const comps = findCompatiblePlayers(manager.players, base.mmr).slice(0, 8);
    if (comps.length >= 2) {
      const room = new MatchmakingRoom(`room_${Date.now()}`);
      manager.players = manager.players.filter((p: Player) => !comps.includes(p));
      comps.forEach((p: Player) => p.setWaiting(room.id));
      distributeToTeams(room, comps);
      comps.forEach((p: Player) => {
        const socket = io.sockets.sockets.get(p.socketId || '');
        if (socket) socket.join(room.id);
      });
      if (comps.length === 8) {
