...
export const useAuthForm = (type: Ref<AuthType>) => {
  ...
  const footerTextKey = computed(() =>
    type.value === 'login' ? `${baseKey.value}.no_account` : `${baseKey.value}.have_account`,
  )
  const linkTextKey = computed(() =>
    type.value === 'login' ? `${baseKey.value}.signup_link` : `${baseKey.value}.login_link`,
  )
  ...
  const handleSubmit = async () => {
    error.value = ''
    if (type.value === 'signup' && form.password !== form.password_confirm)
      return void (error.value = t('auth.signup.password_mismatch'))
    ...
    try {
      ...
      const syncRecord = pb.authStore.record
